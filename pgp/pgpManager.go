...
func MakePGPManager(keyBackend keyBackend.Backend) *PGPManager {
    if keyBackend == nil {
...
func MakePGPManagerWithKRM(keyBackend keyBackend.Backend, krm *KeyRingManager) *PGPManager {
    if keyBackend == nil {
        pgpLog.Fatal("No keyBackend specified")
    }
...
func (pm *PGPManager) LoadKeys() {
    ...
    for _, file := range files {
        ...
        keyData, err := pm.kbkend.Read(file)
        if err != nil {
...
func (pm *PGPManager) generateFastTestKey() (string, error) {
    ...
    e := CreateEntityFromKeys("", "", "", 0, pgpPubKey, pgpPrivKey)
